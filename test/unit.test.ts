...
describe("name game test plan", () => {
  ...
  describe("getGameSession", () => {
    it("it throws GameNotFoundException", async () => {
      ...
      await expect(
        getGameSession(redis)("10RbMxoaP0A6amcAoGeSC2")
      ).to.eventually.be.rejectedWith(
        "Game session 10RbMxoaP0A6amcAoGeSC2 not found"
      );
    ...
    it("gets the session by sessionId", async () => {
      const redis = {
        get: () =>
          Promise.resolve(
            JSON.stringify({
              sessionId: "10RbMxoaP0A6amcAoGeSC2",
      ...
      const spyOnGet = sandbox.spy(redis, "get");
      const session = await getGameSession(redis)("10RbMxoaP0A6amcAoGeSC2");

      expect(spyOnGet).to.have.been.calledOnce;
      expect(session)
        .to.have.property("sessionId")
        .to.equal("10RbMxoaP0A6amcAoGeSC2");
  ...
  describe("createNewHand", () => {
    it("it throws GameNotFoundException", async () => {
      ...
      await expect(
        createNewHand(redis, func)("10RbMxoaP0A6amcAoGeSC2")
      ).to.eventually.be.rejectedWith(
        "Game session 10RbMxoaP0A6amcAoGeSC2 not found"
      );
      expect(spyOnGet).to.have.been.calledOnce;
    ...
    it("creates a new hand", async () => {
      ...
      const hand = await createNewHand(
        redis,
        randomProfiles
  ...
  describe("playHand", () => {
    it("it throws GameNotFoundException", async () => {
      ...
      const spyOnGet = sandbox.spy(redis, "get");
      await expect(
        playHand(redis)("10RbMxoaP0A6amcAoGeSC2", "4NCJTL13UkK0qEIAAcg4IQ")
      ).to.eventually.be.rejectedWith(
        "Game session 10RbMxoaP0A6amcAoGeSC2 not found"
      );
      expect(spyOnGet).to.have.been.calledOnce;
